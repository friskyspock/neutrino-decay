...
class GenDataset:
    ...
    def load_jetfile(self,jet_file):
        ...
        for i in range(self.total_events):
            ...
            for j in range(entry_branch):
                particle_array = [1,0,0,0,root_file.GetLeaf("Photon.PT").GetValue(j),
                                 ...
                                 root_file.GetLeaf("Photon.Eta").GetValue(j),root_file.GetLeaf("Photon.Phi").GetValue(j)]
            ...
            for j in range(entry_branch):
                ...
                particle_array = [0,int(root_file.GetLeaf("Electron.Charge").GetValue(j)),0,0,
                                 ...
                                 root_file.GetLeaf("Electron.Eta").GetValue(j),root_file.GetLeaf("Electron.Phi").GetValue(j)]
                ep_array.append(particle_array)
            ...
            for j in range(entry_branch):
                ...
                particle_array = [0,int(root_file.GetLeaf("Muon.Charge").GetValue(j)),0,0,
                                 ...
                                 root_file.GetLeaf("Muon.Eta").GetValue(j),root_file.GetLeaf("Muon.Phi").GetValue(j)]
                ep_array.append(particle_array)
            ...
            for j in range(entry_branch):
                ...
                particle_array = [0,0,0,1,met_pt,met_pt,0,
                                  root_file.GetLeaf("MissingET.Eta").GetValue(j),
                                  root_file.GetLeaf("MissingET.Phi").GetValue(j)]
                ep_array.append(particle_array)
            ...
            if line != ";\n" or "\n":
                ...
                n21_temp = []
                for i in comb_list[0].split(','):
                    try:
                        n21_temp.append(float(i))
                    except:
                        pass
                n21.append(n21_temp)

                n32_temp = []
                for i in comb_list[1].split(','):
                    try:
                        n32_temp.append(float(i))
                    except:
                        pass
                n32.append(n32_temp)
                        
            event_dict.append({"fourMomenta" : ep_array,"azimuthalAngle" : ez_array,
                              "phiList" : az_angle,"etaList" : ra_angle,"n32" : n32,"n21" : n21})
    ...
    def create_dataset(self,output_file):
        ...
        n21 = []
        for i in range(self.total_events):
            n21.append(self.dataset_dict[i]["n21"])

        n32 = []
        for i in range(self.total_events):
            n32.append(self.dataset_dict[i]["n32"])
        ...
        n21Array = hf.create_group("n21")
        n32Array = hf.create_group("n32")
        ...
        print("INFO : Adding n32 jetdata")
        ak_array = ak.from_iter(n32)
        form, length, container = ak.to_buffers(ak_array,container=n32Array)
        n32Array.attrs["form"] = form.tojson()
        n32Array.attrs["length"] = json.dumps(length)
        n32Array.keys()

        print("INFO : Adding n21 jetdata")
        ak_array = ak.from_iter(n21)
        form, length, container = ak.to_buffers(ak_array,container=n21Array)
        n21Array.attrs["form"] = form.tojson()
        n21Array.attrs["length"] = json.dumps(length)
        n21Array.keys()
    ...
    def pop_dataset(self):
        ...
        None
        '''
        temp_part,temp_az,temp_eta,temp_phi,temp_n21,temp_n32 = self.get_arrays(self.h5_file)
        for i in range(len(temp_part)):
            self.dataset_dict.append({"fourMomenta" : temp_part[i],"azimuthalAngle" : temp_az[i],
                              "phiList" : temp_phi[i],"etaList" : temp_eta[i],"n32" : temp_n32,"n21" : temp_n21})
    ...
    def get_arrays(self,input_file):
        ...
        n21_array = ak.to_list(reconstitutedn21)
        n32_array = ak.to_list(reconstitutedn32)

        print("INFO : Done Getting Data from File")

        return particle_array,az_array,eta_array,phi_array,n21_array,n32_array
    ...
    def add_jetdata(self,fileName):
        self.jet_file = fileName
        f = open(self.jet_file)
        line = f.readline()
        comb_list = line.split(';')
        n21 = []
        n21.append(comb_list[0].split(','))
        n32 = []
        n32.append(comb_list[1].split(','))
